#!/usr/bin/env python
"""
Script pour démarrer l'API ForestAI (avec correctifs) et l'interface web.
Lance le serveur API corrigé et l'application web dans des processus séparés.
"""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description='Démarrer ForestAI API (corrigée) et interface web')
...
def start_api_server(host, port, reload):
    """Démarre le serveur API ForestAI avec les correctifs pour les erreurs de récursion."""
    print(f"Démarrage de l'API ForestAI corrigée sur {host}:{port}...")
    cmd = [sys.executable, "api_server_fix.py", "--host", host, "--port", str(port)]
...
def main():
    ...
    print("\n--- ForestAI est prêt! (Version corrigée) ---")
    if not args.web_only:
