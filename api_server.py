#!/usr/bin/env python
"""
Script de démarrage du serveur API REST ForestAI.

Usage:
    python api_server.py [--host HOST] [--port PORT] [--reload]

Exemples:
    # Démarrer le serveur sur localhost:8000 avec rechargement automatique
    python api_server.py --reload

    # Démarrer le serveur sur une adresse et port spécifiques
...
from forestai.api.server import run_server
...
def parse_arguments():
    ...
    """
    parser = argparse.ArgumentParser(description='ForestAI API REST Server')
...
def main():
    ...
    logger = logging.getLogger("forestai.api_server")
    logger.info("Démarrage du serveur API REST ForestAI")
